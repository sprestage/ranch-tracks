...
class ProjectXmlApiTest < ActionDispatch::IntegrationTest
  ...
  def test_retrieve_project
    ...
    assert_select :tag => "project"
    assert_select :tag => "project", :child => {:tag => "not_done" }
    assert_select :tag => "project", :child => {:tag => "deferred" }
    assert_select :tag => "project", :child => {:tag => "pending" }
    assert_select :tag => "project", :child => {:tag => "done" }
    assert_response 200
  end
